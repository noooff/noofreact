...
const AdminPage = () => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3007/api/products', data, {
        headers: {
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    if (confirmDelete) {
      try {
        const response = await axios.delete(`http://localhost:3007/api/products/${productId}`);
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3007/api/products/${productIdToUpdate}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
